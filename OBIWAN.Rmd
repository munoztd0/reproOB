...
# ERROR: dependency ‘gsl’ is not available for package ‘MBESS’
# * removing ‘/usr/local/lib/R/site-library/MBESS’
# 
# The downloaded source packages are in
# 	‘/tmp/downloaded_packages’
# Error: installation of package ‘MBESS’ had non-zero exit status
...
tables2[1:3,1:3] %>% kbl(caption ="Reaction Times (log)" ) %>%
...
main = lmer(formula, data = PAV.clean, control = control, REML = F)
null = lmer(RT_T ~ condition + (condition|id) + (1|trialxcondition), data = PAV.clean, control = control, REML = F)
test = anova(main, null, test = 'Chisq')
BF_group = exp((test[1,2] - test[2,2])/2); 

main = lmer(formula, data = PAV.clean, control = control, REML = F)
null = lmer(RT_T ~ condition + group + (condition|id) + (1|trialxcondition), data = PAV.clean, control = control, REML = F)
test = anova(main, null, test = 'Chisq')
...
# -------------------------------------- Liking
anova.liking <- aov_car(formula = liking ~ condition*group + Error (id/condition), data = PAV.means, anova_table = list(correction = "GG", es = "pes")); 
``` 
