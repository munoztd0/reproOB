...
```
# Description
Parametric Bootstrap Test method to evaluate significance of fixed effects in mixed-effects models (using MLE fit, nsim = 500) and Bayes Factor from mixed models (see Wagenmakers, 2007)

# Pavlvovian Conditioning Task (Analysis: RT)
Formula = Latency ~ condition*group + age + gender + thirsty + hungry + (condition|id)\
...
Formula = Pleasantness ratings ~ condition*group + age + gender + thirsty + hungry + (1|id)\
...
Formula = Number of grips ~ trial*group +age + gender+ thirsty + hungry  + (trial | id)\
grips = number of times participant exceeded the force threshold to acquire the reward (Milkshake)
Model Comparison between linear, quadratic and piecewise with spline at 5 regressions\
```{r INST, echo=FALSE, include=FALSE, cache=TRUE}
# -------------------------------------- STATS -----------------------------------------------------
# stat -  different fit
formu = '*group +age + gender+ thirsty + hungry'
## LINEAR FIT  
linmod <- lmer(paste('grips~trial', formu, '+ (trial |id)'),data=INST, control = control, REML = FALSE)
## POLYNOMIAL FIT  
quadmod <- lmer(paste('grips~trial+I(trial^2)', formu, '+ (trial+I(trial^2) |id)'), data=INST, control = control, REML = FALSE)
## PIECEWISE REGRESSION WITH SPLINES
splinemod <- lmer(paste('grips ~ lspline(trial, 5)', formu, '+ (lspline(trial, 5) |id)'),  data = INST, control = control, REML = FALSE)
```
\
```{r INST_MS, cache=TRUE}
...
```{r INST_mod, cache=TRUE,  include=FALSE, message=FALSE}

#model = mixed(grips ~ ls1 + ls1:group + ls2 + ls2:group + group + age + gender+ thirsty + hungry + (1 |id), data = INST, method = "LRT", control = control, REML = FALSE) 
#model = mixed(grips ~ ls1 + ls1:group + ls2 + ls2:group + group + age + gender+ thirsty + hungry + (1 |id), data = INST, method = "PB", control = control, REML = FALSE, args_test = list(nsim = 10000, cl=cl)) 

# model = aov_car(grips ~ lspline(trial, 5)*group  + age + gender + thirsty + hungry + Error(id), data= INST, factorize = F, anova_table = list(correction = "GG", es = "none"), fun_aggregate = mean)

model = as_tibble(anova(splinemod))
model$Effect = c('trial-spline(5)', 'group', 'age', 'gender', 'thirsty', 'hungry', 'trial-spline(5)*group') #make it nice
model$df = paste(model$NumDF, signif(model$DenDF,3), sep =', ')
model$MSE = model$`Mean Sq` ; model$`F` = model$`F value`;model$p.value = signif(model$`Pr(>F)` ,2) 
table = model[7:11]; table$p.value[1] = '< 0.001'
# 
# main = splinemod
# null = lmer(paste('grips ~ lspline(trial, 5)', formu, '+ (lspline(trial, 5) |id)'),  data = INST, control = control, REML = FALSE)
...
# test = extractBF(generalTestBF(grips ~ls2*group + age + gender + thirsty + hungry + id, data= INST.means, whichRandom = 'id', neverExclude =  'id', whichModels ="top")); BF = 1/test[1] #switch to BF10 inclusion)); BF = 1/test[1] #switch to BF10 inclusion
# table$BF10 = BF$bf[order(c(11,10,9,8,7,5,4,3,2,1,6))] # reorder
...
tables2 <- as.matrix(tables2) %>% as_tibble()
tables2[is.na(tables2)] <- ""
tables2[1:9,1:4] %>% kbl(caption ="Number of grips", digits = 2) %>%
  kable_styling(latex_options = "HOLD_position", position = "center", full_width = F)  %>%  row_spec(0,bold=T,align='c')

tmp =tables2[10:13,1:2]
names(tmp) <- NULL
tmp %>% kbl() %>%
  kable_styling(latex_options = "HOLD_position", position = "center", full_width = F) 
```
\
```{r INST_BF, echo=FALSE, include=FALSE, cache=TRUE, message=FALSE, warning=FALSE}
#calculate exclusion BF01 for each variable
test = extractBF(generalTestBF(grips ~ ls1 + ls1:group + ls2 + ls2:group + group + age + gender+ thirsty + hungry + id, data= INST, whichRandom = 'id', neverExclude =  'id', whichModels ="top")); BF = 1/test[1] #switch to BF10 inclusion
table = nice(model)
table$BF10 = signif(rev(BF$bf),2)
table$BF10[2] = formatC(rev(BF$bf)[2], format = "e", digits = 2) #formating

table$Effect = c("trial < 5" ,"trial > 5","group", "age", "gender", "thirsty", "hungry", "trial < 5 * group","trial > 5* group")
```
```{r INST_res}
table %>% kbl(caption ="") %>%
  kable_styling(latex_options = "HOLD_position", position = "center", full_width = F) %>%  row_spec(0,bold=T,align='c')
