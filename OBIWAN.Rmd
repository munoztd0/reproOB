...
#create liking covariate
bs = ddply(PAV.clean, .(id, condition), summarise, lik = mean(liking, na.rm = TRUE)) 
CSm = subset(bs, condition == "-1"); CSp = subset(bs, condition == "1"); diff = CSp;
diff$lik = CSp$lik - CSm$lik; diff$lik  = scale(diff$lik)
PAV.clean = merge(x = PAV.clean, y = diff[ , c("lik", 'id')], by = "id", all.x=TRUE)
...
#calculate BF01 for each variable
test = extractBF(generalTestBF(RT ~ condition*group + age + gender + thirsty + hungry + id, data= PAV.clean, whichRandom = 'id', neverExclude =  'id')); BF = 1/test[1] #switch to BF10
